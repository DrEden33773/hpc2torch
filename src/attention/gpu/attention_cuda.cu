...
__device__ void matmulRQK(const float *__restrict inputQ,
  ...
  for (int index = 0; index < Bd; index++) {
    (float4 &)com_a[0] = (float4 &)
        shareQK[index * numQ + threadIdx.y * Rq + (ph - 1) % 2 * numQ * Bd];
    (float4 &)com_a[4] = (float4 &)
        shareQK[index * numQ + threadIdx.y * Rq + 4 + (ph - 1) % 2 * numQ * Bd];
    (float4 &)com_b[0] = (float4 &)
        shareVK[index * numK + threadIdx.x * Bk + (ph - 1) % 2 * numK * Bd];
    (float4 &)com_b[4] = (float4 &)
        shareVK[index * numK + threadIdx.x * Bk + 4 + (ph - 1) % 2 * numK * Bd];
  ...
  for (int index_k = 0; index_k < Bk; index_k++) {
    for (int id = 0; id < Rv; id += 4) {
      (float4 &)shareVK[threadIdx.y * numV + threadIdx.x * Rv + id] =
          (float4 &)inputV[(indK + threadIdx.y * Bk + index_k) * d + indV +
                           threadIdx.x * Rv + id];
    }
...
__global__ void _attentionKernel(const float *__restrict inputQ,
  ...
  for (int j = 0; j < Tc; j++) {
    ...
    matmulRQK(inputQ, inputK, shareQK, shareVK, N, d, width, indQ, indK, val);
    for (int index_q = 0; index_q < Rq; index_q++) {
      ...
      for (int index_k = 0; index_k < Bk; index_k++) {
        if (indQ + threadIdx.y * Rq + index_q < N &&
          ...
          tmpReduceMax = max(tmpReduceMax, val[index_q * Bk + index_k]);
        }
      }
      __syncthreads();
      ...
      if (newMax[index_q] > block_max[threadIdx.y * Rq + index_q]) {
        newSum[index_q] = std::fma(
            ...
            __expf(block_max[threadIdx.y * Rq + index_q] - newMax[index_q]),
            newSum[index_q]);
      } else {
        newSum[index_q] = std::fma(
            newSum[index_q],
            __expf(newMax[index_q] - block_max[threadIdx.y * Rq + index_q]),
            block_sum[threadIdx.y * Rq + index_q]);
    ...
    matmulSV(shareQK, inputV, shareVK, N, d, j, indQ, indK, indV, val, newMax,
             sumSV);
  ...
  for (int index_q = 0; index_q < Rq; index_q++) {
    ...
    for (int id = 0; id < Rv; id += 4) {
      if (indQ + threadIdx.y * Rq + index_q < N &&
        ...
        (float4 &)output[(indQ + threadIdx.y * Rq + index_q) * d + indV +
                         threadIdx.x * Rv + id] =
...
extern "C" void attention_nv_f32(void const *inputQ, void const *inputK,
                                 void const *inputV, int N, int d,
                                 void *output) {
  ...
  _attentionKernel<Br, Bc, Rq, Rv><<<grid_dim, block_dim>>>(
      (float *)inputQ, (float *)inputK, (float *)inputV, N, d, (float *)output);
