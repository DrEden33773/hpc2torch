...
#include <stdio.h>
template <typename T>
void layernorm_cpu(void const *input, void const *scale, void const *bias,
  ...
  if (sizeof(T) == 4) {
    for (int i = 0; i < frontsize; i++) {
      ...
      for (int id = 0; id < behindsize; id++) {
        destination[tid + id] =
            (source[tid + id] - mu) * weight[id] * sigma2 + _bias[id];
      }
    }
  } else if (sizeof(T) == 2) {
    for (int i = 0; i < frontsize; i++) {
      ...
      float sigma2Partial = 0.0f;
      for (int id = 0; id < behindsize; id++) {
        sigma2Partial += (f16_to_f32(source[tid + id]) - mu) *
                         (f16_to_f32(source[tid + id]) - mu);
      }
      float sigma2 = 1.0f / sqrt(sigma2Partial / behindsize + eps);
      for (int id = 0; id < behindsize; id++) {
        float tmp = (f16_to_f32(source[tid + id]) - mu) *
                        f16_to_f32(weight[id]) * sigma2 +
                    f16_to_f32(_bias[id]);
...
extern "C" void layernorm_cpu_f32(void const *input, void const *scale,
...
extern "C" void layernorm_cpu_f16(void const *input, void const *scale,
                                  void const *bias, void *output, float eps,
                                  int size, int behindsize) {
