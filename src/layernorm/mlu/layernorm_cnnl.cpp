...
template <typename T>
void layernormCnnlDevice(void const *source, void const *weight,
                         void const *bias, void *destination, int *shape,
                         int nDim, int axis, float eps, cnnlHandle_t &handle,
                         cnrtQueue_t &queue) {
  ...
  cnnlSetTensorDescriptor(xDesc, CNNL_LAYOUT_ARRAY, dataType, inDim.size(),
                          inDim.data());
  cnnlSetTensorDescriptor(yDesc, CNNL_LAYOUT_ARRAY, dataType, outDim.size(),
                          outDim.data());
  cnnlSetTensorDescriptor(filter_bias_desc, CNNL_LAYOUT_ARRAY, dataType,
                          filter_biasDim.size(), filter_biasDim.data());
  cnnlSetTensorDescriptor(mean_rstd_desc, CNNL_LAYOUT_ARRAY, dataType,
  ...
  cnnlLayerNormForward(handle, xDesc, source, axis, filter_bias_desc, weight,
                       bias, eps, workspace, wsSize, yDesc, destination,
                       mean_rstd_desc, mean_dev, rstd_dev);
...
template <typename T>
void layernormCnnl(void const *input, void const *scale, void const *bias,
                   void *output, int *shape, int nDim, int axis, float eps) {
  ...
  layernormCnnlDevice<T>(input, scale, bias, output, shape, nDim, axis, eps,
                         handle, queue);

  cnnlDestroy(handle);
  CNRT_CHECK(cnrtQueueDestroy(queue));
}
extern "C" void layernorm_cnnl_f32(void const *input, void const *scale,
                                   void const *bias, void *output, int *shape,
                                   int nDim, int axis, float eps) {
  layernormCnnl<float>(input, scale, bias, output, shape, nDim, axis, eps);
}
extern "C" void layernorm_cnnl_f16(void const *input, void const *scale,
                                   void const *bias, void *output, int *shape,
                                   int nDim, int axis, float eps) {
