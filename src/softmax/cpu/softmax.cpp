...
#include <stdio.h>
template <typename T>
void softmax_cpu(void const *input, void *output, int size, int dimsize,
                 int stride) {
  ...
  if (sizeof(T) == 4) {
    ...
    for (int ind = 0; ind < othersize; ind++) { // ind = i(KS) + k(S) + s
      int tid = ind % stride +
                (ind - ind % stride) * dimsize; // now, tid = i(JKS) + k(S) + s;
    ...
    for (int ind = 0; ind < othersize; ind++) { // ind = i(KS) + k(S) + s
      ...
      for (int j = 0; j < dimsize; j++) {
        int index = tid + j * stride;
        destination[index] =
            f32_to_f16(std::exp(f16_to_f32(source[index]) - localM) / localS);
      }
    }
  }
}
extern "C" void softmax_cpu_f32(void const *input, void *output, int size,
                                int dimsize, int stride) {
  softmax_cpu<float>(input, output, size, dimsize, stride);
}
extern "C" void softmax_cpu_f16(void const *input, void *output, int size,
                                int dimsize, int stride) {
