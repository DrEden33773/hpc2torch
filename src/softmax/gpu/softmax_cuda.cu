...
__launch_bounds__(BLOCK_DIM) __global__ void _blockSoftmaxKernel(
  ...
  int tid = blockIdx.x % stride + (blockIdx.x - blockIdx.x % stride) *
  ...
  if (threadIdx.x < remain) {
    ...
    for (int ind = 0; ind < step - 1; ind++) {
      dms_input.max_tmp = static_cast<float>(
          input[tid +
  ...
  if (threadIdx.x < remain) {
    ...
    for (int ind = 0; ind < step - 1; ind++) {
      ...
      output[tid + (remain * step + (threadIdx.x - remain) * (step - 1) + ind) *
                       stride] =
          static_cast<T>(
              __expf(
                  static_cast<float>(
...
__global__ void
_blockSoftmaxKernel(T const *input, T *output, int dimsize,
                    int stride) { // if set axis = 1, inputShape=[I,J,K,S]
                                  // tid = i(JKS) + j(KS) + k(S) + s
  ...
  if (threadIdx.x < remain) {
    ...
    for (int ind = 0; ind < step - 1; ind++) {
      dataPerThread[ind] = static_cast<float>(
          input[tid +
                (remain * step + (threadIdx.x - remain) * (step - 1) + ind) *
                    stride]);
      dms_input.max_tmp = dataPerThread[ind];
  ...
  if (threadIdx.x < remain) {
    for (int ind = 0; ind < step; ind++) {
      output[tid + (threadIdx.x * step + ind) * stride] =
          static_cast<T>(__expf(dataPerThread[ind] - dms_total.max_tmp) *
                         __fdividef(1.0F, dms_total.sum_tmp));
    }
  } else {
    for (int ind = 0; ind < step - 1; ind++) {
...
__global__ void _warpSoftmaxKernel(T const *input, T *output, int othersize,
  ...
  if (otherIdx < othersize) {
    ...
    for (int ph = 0; threadIdx.x + ph * BLOCK_DIM_x < dimsize; ph++) {
      dataPerThreadx[ph] = static_cast<float>(
...
//-----------------
template <typename T>
void softmaxLaunch(void const *input, void *output, int size, int dimsize,
                   int stride) {
  ...
  if (dimsize > 1024 * 128) {
    ...
    _warpSoftmaxKernel<T, 32, 32, 32><<<grid_dim, block_dim>>>(
        (T *)input, (T *)output, num_blocks, dimsize, stride);
  } else if (dimsize > 15) {
    int BLOCK_DIM_x = 16;
    int BLOCK_DIM_y = 64;
    int num_block_x = (num_blocks + BLOCK_DIM_y - 1) / BLOCK_DIM_y;
    ...
    _warpSoftmaxKernel<T, 16, 64, 2><<<grid_dim, block_dim>>>(
        (T *)input, (T *)output, num_blocks, dimsize, stride);
  } else if (dimsize > 7) {
    int BLOCK_DIM_x = 8;
    int BLOCK_DIM_y = 128;
    ...
    _warpSoftmaxKernel<T, 8, 128, 2><<<grid_dim, block_dim>>>(
        (T *)input, (T *)output, num_blocks, dimsize, stride);
  } else {
    int BLOCK_DIM_x = 4;
    int BLOCK_DIM_y = 256;
    ...
    _warpSoftmaxKernel<T, 4, 256, 2><<<grid_dim, block_dim>>>(
        (T *)input, (T *)output, num_blocks, dimsize, stride);
  }
  cudaDeviceSynchronize();
}
extern "C" void softmax_nv_f32(void const *input, void *output, int size,
                               int dimsize, int stride) {
  softmaxLaunch<float>(input, output, size, dimsize, stride);
}
extern "C" void softmax_nv_f16(void const *input, void *output, int size,
                               int dimsize, int stride) {
