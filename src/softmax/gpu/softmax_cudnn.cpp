...
template <typename T>
void softmaxCudnnDevice(cudnnHandle_t &handle, void const *input, void *output,
                        int *shape, int ndim) {
  ...
  cudnnSetTensor4dDescriptor(inputDesc, CUDNN_TENSOR_NCHW, cudnnDataType,
                             dim_array[0], dim_array[1], dim_array[2],
                             dim_array[3]);
  cudnnSetTensor4dDescriptor(outputDesc, CUDNN_TENSOR_NCHW, cudnnDataType,
                             dim_array[0], dim_array[1], dim_array[2],
                             dim_array[3]);
  float alpha = 1.0;
  float beta = 0.0;
  ...
  cudnnSoftmaxForward(handle, algo, mode, &alpha, inputDesc, input, &beta,
                      outputDesc, output);
...
extern "C" void softmax_cudnn_f32(void const *input, void *output, int *shape,
                                  int ndim) {
  softmaxCudnn<float>(input, output, shape, ndim);
}
extern "C" void softmax_cudnn_f16(void const *input, void *output, int *shape,
                                  int ndim) {
