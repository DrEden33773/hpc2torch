...
template <typename T>
void softmaxCnnlDevice(T const *source, T *destination, int nDim, int axis,
                       int *shape, cnnlHandle_t &handle, cnrtQueue_t &queue) {
  ...
  cnnlStatus_t stat = cnnlSoftmaxForward_v2(
      handle, CNNL_SOFTMAX_ACCURATE, mode, CNNL_COMPUTATION_ULTRAHIGH_PRECISION,
...
template <typename T>
void softmaxCnnl(void const *input, void *output, int nDim, int axis,
                 int *shape) {
...
extern "C" void softmax_cnnl_f32(void const *input, void *output, int nDim,
                                 int axis, int *shape) {
  softmaxCnnl<float>(input, output, nDim, axis, shape);
}
extern "C" void softmax_cnnl_f16(void const *input, void *output, int nDim,
                                 int axis, int *shape) {
